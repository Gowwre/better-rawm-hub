...
export function send_event_config_reset(client) {
  var payload = [];
  payload.push(0x3);
  payload.push(0x0);
  payload.push(0x3);
...
export function send_event_factory_reset(client, isReboot) {
  send_event_action(client, CMD_FACTORY_RESET, isReboot ? 0x1 : 0x0);
  client.device_info.pollingRate = -0x1;
}
export function query_firmware(client, fwChannel) {
...
export function upload_mouse_config(client, value, value2) {
  var xhr = new XMLHttpRequest();
  xhr.onreadystatechange = function () {
    if (this.readyState == 0x4 && this.status == 0xc8) {
      try {
...
export function upload_mouse_config_delayed(deviceInfo, channel, sleepTime) {
