...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
hidHandlers[RESP_PARAMETER] = function hid_parse_parameter(client: any, byteLen: any, value2: any) {
  ...
  if (value4 == PARAM_RESOLUTION) {
    ...
    for (var offset = 0; offset < bytes.byteLength; offset++) {
      if (bytes[offset] == 0) {
        client.device_info.colorCode = String.fromCharCode.apply(null, Array.from(bytes.subarray(0, offset)));
        window.postMessage({ 'action': ACTION_UI_REFRESH_CLIENT_LIST });
        window.postMessage({ 'action': ACTION_UI_REFRESH_CURRENT_CLIENT });
        break;
      }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
hidHandlers[RESP_PING] = function hid_parse_ping(client: any, byteLen: any, value2: any) {
  log_r("PING <");
  if (!client.connected) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
hidHandlers[RESP_SYNC] = function hid_parse_sync(client: any, byteLen: any, value2: any) {
