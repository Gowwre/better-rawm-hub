...
import { do_resize, setting_mapping_key_recording_add, setting_mapping_macro_recording_add } from './event-dispatch.js';
...
export function kbd_ui_macro_edit_init(client) {
  ...
  for (var i = 0; i < S.edit_macros.length; i++) {
    ...
    if (event == MOUSE_EVENT_WHEEL_VERT) {
      ...
      var px = Math.floor((code >> 16 & 0xffff) * screenW / 0xffff);
      var py = Math.floor((code & 0xffff) * screenH / 0xffff);
  ...
  if (S.setting_mapping_key_recording) {
    ...
    if (S.setting_macro_edit_recording) {
      if (S.edit_macros.length >= 0xc8) {
        layui.layer.msg(layui.i18np.prop("STRID_SETTING_MAPPING_MACRO_RECORD_MAX_EX"), {
          'icon': 0x0
        }, function () {});
        return;
      }
...
layui.use(['form', "layer", 'util', "i18np", "table"], function () {
  ...
  layui2.on("switch(x-polling)", function (result) {
    if (result.elem.checked) {
      localStorage.setItem('setting-x-polling', String(0x1));
    } else {
      localStorage.setItem('setting-x-polling', String(0x0));
    }
    window.postMessage({
      'action': ACTION_UI_REFRESH_SETTING
    });
  ...
  layui4("#mapping-key-turbo-freq-input").on('input', function (result) {
    ...
    var parsedInt1 = len5.length == 0x0 ? 0x0 : Math.floor(len5);
    if (parsedInt1 <= 0x0) {
      parsedInt1 = 0x1;
    }
    if (parsedInt1 != Math.floor(0x3e8 / (keyInfo8.mouse_auto_click_down + keyInfo8.mouse_auto_click_up))) {
      var value37 = Math.floor(0x3e8 / parsedInt1);
      if (value37 >= 0x64) {
        keyInfo8.mouse_auto_click_down = 0x32;
        keyInfo8.mouse_auto_click_up = value37 - keyInfo8.mouse_auto_click_down;
      } else {
        keyInfo8.mouse_auto_click_up = Math.floor(value37 / 0x2);
  ...
  layui4("#mapping-key-turbo-rand-input").on("input", function (result) {
    ...
    var parsedInt2 = len6.length == 0x0 ? 0x0 : Math.floor(len6);
    if (parsedInt2 < 0x0) {
      parsedInt2 = 0x0;
    }
  ...
  layui4("#mapping-key-turbo-down-keep-input").on("input", function (result) {
    ...
    var parsedInt3 = len7.length == 0x0 ? 0x0 : Math.floor(len7);
    if (parsedInt3 < 0x0) {
      parsedInt3 = 0x0;
    }
    ...
    layui4("#mapping-key-turbo-freq-input").val(Math.floor(0x3e8 / value38));
    S.need_save = true;
    ui_refresh_onboard_config(DS.current_usb_client);
  });
  ...
  layui4("#mapping-key-turbo-up-keep-input").on("input", function (result) {
    ...
    var parsedInt4 = len8.length == 0x0 ? 0x0 : Math.floor(len8);
    if (parsedInt4 < 0x0) {
      parsedInt4 = 0x0;
    }
    ...
    layui4("#mapping-key-turbo-freq-input").val(Math.floor(0x3e8 / value39));
    S.need_save = true;
    ui_refresh_onboard_config(DS.current_usb_client);
  });
    ...
    'select': async function () {
      ...
      el.open({
        ...
        'btn2': function () {
          ...
          var macroInfo = create_macro_info();
          macroInfo.style = 0x16;
          var value40 = macro_keys[Math.floor(layui4("[name=\"macro-add-select-key\"]").val())].vCode;
          if (DS.current_usb_client != undefined ? is_hs_keyboard(DS.current_usb_client.device) : false) {
            value40 = get_key_code_from_name(document.getElementById("kbd-macro-add-select-key").textContent);
          }
          if (value40 == 0x401) {
            macroInfo.mouse_key_event = 0x20a;
            macroInfo.mouse_key_code = -Math.floor(layui4("#macro-add-wheel-delta-input").val());
          } else {
            if (value40 == 0x400) {
              macroInfo.mouse_key_event = 0x20a;
              macroInfo.mouse_key_code = Math.floor(layui4("#macro-add-wheel-delta-input").val());
              ...
              if (value40 == 0x402) {
                ...
                macroInfo.mouse_key_code = -Math.floor(layui.$("#macro-add-wheel-delta-input").val());
              } else {
                if (value40 == 0x403) {
                  macroInfo.mouse_key_event = 0x20e;
                  macroInfo.mouse_key_code = Math.floor(layui4('#macro-add-wheel-delta-input').val());
                  ...
                  if (value40 == 0x404) {
                    ...
                    macroInfo.mouse_key_loop = Math.floor(layui4('#macro-add-move-loop-input').val());
                    ...
                    if (value40 == 0x405) {
                      ...
                      var value43 = Math.floor(layui4("#macro-add-position-x-input").val());
                      var value44 = Math.floor(layui4('#macro-add-position-y-input').val());
                      var screenW = window.screen.width;
                      var screenH = window.screen.height;
                      value43 = Math.floor((value43 + 0.9) * 0xffff / screenW);
                      value44 = Math.floor((value44 + 0.9) * 0xffff / screenH);
        ...
        'btn3': function () {
          ...
          var value45 = macro_keys[Math.floor(layui4("[name=\"macro-add-select-key\"]").val())].vCode;
          if (DS.current_usb_client != undefined ? is_hs_keyboard(DS.current_usb_client.device) : false) {
          ...
          if (value45 == 0x401) {
            ...
            S.current_edit_macro.mouse_key_code = -Math.floor(layui4("#macro-add-wheel-delta-input").val());
          } else {
            if (value45 == 0x400) {
              S.current_edit_macro.mouse_key_event = 0x20a;
              S.current_edit_macro.mouse_key_code = Math.floor(layui4("#macro-add-wheel-delta-input").val());
              ...
              if (value45 == 0x402) {
                ...
                if (value45 == 0x403) {
                  ...
                  if (value45 == 0x404) {
                    ...
                    S.current_edit_macro.mouse_key_loop = Math.floor(layui4('#macro-add-move-loop-input').val());
                    ...
                    if (value45 == 0x405) {
                      ...
                      var value48 = Math.floor(layui4("#macro-add-position-x-input").val());
                      var value49 = Math.floor(layui4('#macro-add-position-y-input').val());
                      var screenW = window.screen.width;
                      var screenH = window.screen.height;
                      value48 = Math.floor((value48 + 0.9) * 0xffff / screenW);
                      value49 = Math.floor((value49 + 0.9) * 0xffff / screenH);
        ...
        'btn2': function () {
          ...
          el.open({
            ...
            'btn1': function () {
              el.closeLast(0x0);
              S.current_edit_macro.style = 0x16;
              var value51 = macro_keys[Math.floor(layui4("[name=\"macro-add-select-key\"]").val())].vCode;
              if (value51 == 0x401) {
                S.current_edit_macro.mouse_key_event = 0x20a;
                S.current_edit_macro.mouse_key_code = -Math.floor(layui4("#macro-add-wheel-delta-input").val());
              } else {
                  ...
                  if (value51 == 0x402) {
                    ...
                    if (value51 == 0x403) {
                      ...
                      if (value51 == 0x404) {
                        ...
                        if (value51 == 0x405) {
                          S.current_edit_macro.mouse_key_event = 0x2ff;
                          var value54 = Math.floor(layui4("#macro-add-position-x-input").val());
                          var value55 = Math.floor(layui4("#macro-add-position-y-input").val());
                          var screenW = window.screen.width;
                          var screenH = window.screen.height;
                          value54 = Math.floor((value54 + 0.9) * 0xffff / screenW);
                          value55 = Math.floor((value55 + 0.9) * 0xffff / screenH);
  ...
  layui4("#macro-add-wheel-delta-input").on('input', function (result) {
    ...
    var parsedInt5 = len10.length == 0x0 ? 0x0 : Math.floor(len10);
    if (parsedInt5 < 0x0) {
      parsedInt5 = 0x0;
    }
  ...
  layui4("#color-r-input").on('input', function (result) {
    ...
    var value99 = Math.floor(value98);
    var value100 = Math.floor(layui4("#color-g-input").val());
    var value101 = Math.floor(layui4("#color-b-input").val());
  ...
  layui4('#color-g-input').on("input", function (result) {
    ...
    var value104 = Math.floor(layui4("#color-r-input").val());
    var value105 = Math.floor(value103);
    var value106 = Math.floor(layui4("#color-b-input").val());
  ...
  layui4("#color-b-input").on("input", function (result) {
    ...
    var value109 = Math.floor(layui4("#color-r-input").val());
    var value110 = Math.floor(layui4("#color-g-input").val());
    var value111 = Math.floor(value108);
  ...
  layui4("#light-box-color-r-input").on("input", function (result) {
    ...
    var value114 = Math.floor(value113);
    var value115 = Math.floor(layui4("#light-box-color-g-input").val());
    var value116 = Math.floor(layui4('#light-box-color-b-input').val());
  ...
  layui4("#light-box-color-g-input").on("input", function (result) {
    ...
    var value118 = Math.floor(layui4("#light-box-color-r-input").val());
    var value119 = Math.floor(value117);
    var value120 = Math.floor(layui4("#light-box-color-b-input").val());
  ...
  layui4("#light-box-color-b-input").on("input", function (result) {
    ...
    var value122 = Math.floor(layui4("#light-box-color-r-input").val());
    var value123 = Math.floor(layui4("#light-box-color-g-input").val());
    var value124 = Math.floor(value121);
    ...
    'select': async function () {
      ...
      el.open({
        ...
        'btn1': function () {
          el.closeLast(0x0);
          S.current_edit_macro.style = 0x16;
          var value144 = macro_keys[Math.floor(layui4("[name=\"macro-add-select-key\"]").val())].vCode;
          if (value144 == 0x401) {
            S.current_edit_macro.mouse_key_event = 0x20a;
            S.current_edit_macro.mouse_key_code = -Math.floor(layui4("#macro-add-wheel-delta-input").val());
          } else {
              ...
              if (value144 == 0x402) {
                ...
                if (value144 == 0x403) {
                  ...
                  if (value144 == 0x404) {
                    ...
                    if (value144 == 0x405) {
                      S.current_edit_macro.mouse_key_event = 0x2ff;
                      var value147 = Math.floor(layui4("#macro-add-position-x-input").val());
                      var value148 = Math.floor(layui4("#macro-add-position-y-input").val());
                      var screenW = window.screen.width;
                      var screenH = window.screen.height;
                      value147 = Math.floor((value147 + 0.9) * 0xffff / screenW);
                      value148 = Math.floor((value148 + 0.9) * 0xffff / screenH);
  ...
  layui3.on("kbd-macro-save-action", {
    'select': async function () {
      ...
      if (S.kbd_macro_select_index >= 0x0) {
          ...
          if (len36.length > 0x0) {
            for (var len37 = 0x0; len37 < len36.length; len37++) {
              ...
              if (value149.mouse_key_event == 0x100) {
                offset17 = 0x1;
                DeviceStore.kbdSync.macroBuff.push(offset17);
                offset17 = 0x2;
                DeviceStore.kbdSync.macroBuff.push(offset17);
        ...
        log_r(DeviceStore.kbdSync.macroBuff);
        if (DeviceStore.kbdSync.macroBuff.length > DS.current_usb_client.device_info.kbd_macro_max_size) {
  ...
  var _platform: any = (window as any).platform || {}; if (_platform.os && _platform.os.family && _platform.os.family.indexOf("Windows") >= 0x0 && parseFloat(_platform.os.version) <= 6.1) {
